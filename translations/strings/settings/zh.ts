export const zh = {
  settings: {
    ...
    'new-version-available-notif-info':
      'Essentials 有新的版本 ({{ latestVersion }})，请尽快更新以获取最新的功能和问题修复！',
    ...
    'developer-bitcoin-signdata-prompt':
      '如果启用此设置，您可能会收到不可读的签名请求。 通过签署一条您不理解的消息，您可能同意放弃您的资金。 您面临网络钓鱼攻击的风险。 通过关闭signData 来保护自己。',
    'developer-logs': '捕获日志',
    ...
    'wallet-connect-no-session-info':
      '当前没有通过 Wallet Connect 与第三方应用程序的会话。请在外部应用程序上找到 Wallet Connect 按钮，然后使用 Essentials 的扫描仪扫描此二维码以建立会话连接。',
    'wallet-connect-no-address':
      '没有活跃的钱包，或者活跃的网络与以太坊不兼容。这不是错误，但请确保您在预期的网络上。 否则，某些 dApp 可能会断开您的 Wallet Connect 会话。',
    ...
    'hive-data-sync': '数据同步',
    'privacy-use-hive-data-sync': '使用我的 Elastos Hive Vault 存储空间来保存和恢复个人数据，例如 DID 凭证或联系人列表',
    'privacy-dont-use-hive-data-sync': '不使用我的 Elastos Hive Vault 存储空间来保存和恢复个人数据',
    ...
    'trinity-tech-io-des':
      '由Trinity技术团队部署和维护的一套Elastos API，Trinity技术团队负责Elastos SDK和Essentials的开发。',

    'privacy-toolbox-stats': '凭证统计信息',
    'privacy-send-credential-toolbox-stats':
      '将有关DID使用情况的匿名信息发送到凭证工具箱(开发人员工具)。DID本身永远不会发送。',
