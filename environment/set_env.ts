...
const devEnv = {
  production: false,
  useTestPolls: parseBoolean(localEnv.USE_TEST_POLLS ?? env.USE_TEST_POLLS) ?? true,
  EssentialsAPI: {
...
if (!existsSync('./src/environments')) mkdirSync('./src/environments');
...
if (prodEnv.NownodesAPI.apikey.length === 0) {
  console.warn(
    yellow(
      'Warning: The nownodes apikey for production is empty! You will not be able to obtain relevant data of the btc chain'
    )
  );
}
