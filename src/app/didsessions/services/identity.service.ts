...
import { DIDSessionRoute, UXService } from './ux.service';
...
export class IdentityService {
  ...
  public async signIn(identityEntry: IdentityEntry, goToLauncher = false, showSignInDialog = false) {
    ...
    try {
      ...
      let passwordInfo = await this.globalPasswordService.getPasswordInfo(
      ...
      if (passwordInfo) {
        ...
        Logger.log(
          'didsessions',
          'Language changed by user. Passing session language to be: ' + this.language.activeLanguage.value
        );
        signInOptions = {
      ...
      try {
        ...
        if (e) {
          ...
          if (message.indexOf('signature mismatch') > 0) {
            ...
            await this.nativeService.hideLoading();
            void this.globalPopupService
              .ionicAlert('didsessions.did-corrupted-title', 'didsessions.did-corrupted-info', 'didsessions.got-it')
              .then(() => {
  ...
  private navigateWithCompletion(
    route: (typeof DIDSessionRoute)[number],
    completionCb: stepCompletionCallback,
    enterEventData?: any
  ) {
    this.nextStepId++;
