...
export class GlobalNavService {
  ...
  public navigateHome(direction = Direction.BACK): Promise<boolean> {
    ...
    return this.zone.run(() => {
      if (direction != Direction.NONE)
        // No animation - ex for the first arrival on the launcher home
        return this.navCtrl.navigateRoot(launcherHome.route, { animationDirection: direction });
      else return this.navCtrl.navigateRoot(launcherHome.route);
    });
  }
  ...
  public navigateTo(context: string, route: string, routerOptions?: NavigationOptions): Promise<boolean> {
    ...
    if (!context) context = this.navigationHistory[this.navigationHistory.length - 1].context;
  ...
  public navigateBack(): Promise<boolean> {
    if (!this.canGoBack()) throw new Error('Unable to navigate back. No more known route in stack');
  ...
  public async exitCurrentContext(navigate = true): Promise<void> {
    ...
    while (this.canGoBack()) {
      ...
      if (currentStep.context == startContext) this.navigationHistory.pop(); // Same context, unstack the step
      else {
        if (navigate) {
  ...
  public showRestartPrompt(showCancelButton = false, customMessage = ''): Promise<void> {
