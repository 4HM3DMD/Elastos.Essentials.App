...
import {
  ElastosPaginatedTransactions,
  TransactionDetail,
  UtxoType
} from '../wallet/model/tx-providers/transaction.types';
...
export class GlobalElastosAPIService extends GlobalService {
    ...
    this.availableProviders = [
      {
        ...
        endpoints: {
          MainNet: {
            ...
            ecoPGPRPC: 'https://api.elastos.io/pg',
  ...
  async onUserSignIn(signedInIdentity: IdentityEntry): Promise<void> {
    ...
    let providerName = (await this.prefs.getPreference(
  ...
  /**
   * Returns the right RPC API url to use by trying to ask the wallet, so we get the overridden
   * rpc urls.
   *
   * BEWARE when calling getApiUrl() from here, this can cause an infinite loop...
   */
  public getRPCApiUrlWithOverride(type: ElastosApiUrlType): string {
    let network: AnyNetwork;
    switch (type) {
      case ElastosApiUrlType.ELA_RPC:
        network = this.networksService.getNetworkByKey('elastos');
        break;
      case ElastosApiUrlType.EID_RPC:
        network = this.networksService.getNetworkByKey('eid');
        break;
    ...
    // No network? we might be not signed in. In this case, just use the default hardcoded value
    // provided by this service.
    if (!network) {
      return this.getApiUrl(type);
    }

    // This network method might return the user overriden network, or call getApiUrl() from this
    // service.
    return network.getRPCUrl();
  }
   ...
   *
   * IMPORTANT: Should only be called by networks to implements getAPIUrlOfType(). Other services must call
   * getRPCApiUrlWithOverride() instead.
   */
  public getApiUrl(
    type: ElastosApiUrlType,
    activeNetworkTemplate = this.globalNetworksService.activeNetworkTemplate.value
  ): string {
    ...
    if (!(type in endpoints)) {
      Logger.warn(
        'elastosapi',
        'Elastos API provider ' + activeProvider.name + ' does not support url type ' + type + '!'
      );
      return null;
    }
  ...
  private callTestAPIOnProvider(provider: ElastosAPIProvider): Promise<void> {
    ...
    return new Promise(async resolve => {
      ...
      try {
        ...
        Logger.log(
          'elastosapi',
          'Provider ' + provider.name + ' just answered the test api call with value (block height) ',
          data
        );
        // Set the provider as best provider if no one did that yet. We are the fastest api call to answer.
        if (!this._bestProvider) this._bestProvider = provider;
        resolve();
      } catch (e) {
  ...
  private async setResolverUrl(): Promise<void> {
    let didResolverUrl = this.getRPCApiUrlWithOverride(ElastosApiUrlType.EID_RPC);
  ...
  public getApiUrlForChainCode(elastosChainCode: StandardCoinName): string {
    return this.getRPCApiUrlWithOverride(this.getApiUrlTypeForRpc(elastosChainCode));
  }
  ...
  async getRawTransaction(txid: string, elastosChainCode = StandardCoinName.ELA): Promise<TransactionDetail> {
    ...
    const rpcApiUrl = this.getRPCApiUrlWithOverride(this.getApiUrlTypeForRpc(elastosChainCode));
    if (rpcApiUrl === null) {
      return null;
    }
  ...
  public async getTransactionsByAddress(
    elastosChainCode: StandardCoinName,
    addressArray: string[],
    limit: number,
    skip = 0,
    timestamp = 0
  ): Promise<{ result: ElastosPaginatedTransactions }[]> {
    const paramArray = [];
    let index = 0;
  ...
  public async getERC20TokenTransactions(
    elastosChainCode: StandardCoinName,
    address: string
  ): Promise<EthTokenTransaction[]> {
  ...
  public async getAllUtxoByAddress(
    elastosChainCode: StandardCoinName,
    addresses: string[],
    utxotype: UtxoType = UtxoType.Mixed
  ): Promise<any> {
    const param = {
      method: 'listunspent',
      params: {
  ...
  public async getCurrentHeight() {
    ...
    if (this.blockHeightCache) {
      ...
      if (current - this.blockHeightTimestamp < 60000) {
        // 60s
    ...
    const rpcApiUrl = this.getRPCApiUrlWithOverride(ElastosApiUrlType.ELA_RPC);
    if (rpcApiUrl === null) {
      return 0;
    }
  ...
  public async fetchCRcouncil(index = 0): Promise<CRCouncilSearchResponse> {
    const rpcApiUrl = this.getRPCApiUrlWithOverride(ElastosApiUrlType.CR_RPC);
  ...
  public async fetchImages(): Promise<ImageInfos> {
    ...
    const rpcApiUrl = this.getRPCApiUrlWithOverride(ElastosApiUrlType.IMAGES);
