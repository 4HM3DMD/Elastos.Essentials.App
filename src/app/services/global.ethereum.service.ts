...
export class GlobalEthereumRPCService {
  ...
  public async eth_getBalance(
    rpcApiUrl: string,
    address: string,
    limitatorName = 'default',
    highPriority = false
  ): Promise<BigNumber> {
    const param = {
    ...
    try {
      ...
      let balanceString = await this.globalJsonRPCService.httpPost(
        rpcApiUrl,
        param,
        limitatorName,
        10000,
  ...
  public eth_sendRawTransaction(rpcApiUrl: string, txHash: string, limitatorName = 'default') {
    if (!txHash) throw new Error("eth_sendRawTransaction(): transaction hash can't be empty!");
