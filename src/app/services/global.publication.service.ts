...
abstract class DIDPublisher {
  public abstract publishDID(
    didString: string,
    payloadObject: any,
    memo: string,
    showBlockingLoader: boolean,
    parentIntentId?: number
  ): Promise<void>;
...
namespace AssistPublishing {
  ...
  export class AssistPublisher extends DIDPublisher {
    ...
    public async publishDID(
      ...
      if (typeof payloadObject === 'string') throw new Error('Payload must be a JSON object, not a stringified JSON');
      ...
      try {
        ...
        let response: AssistCreateTxResponse = (await this.http
          .post(assistAPIEndpoint + '/didtx/create', requestBody, {
            headers: headers
          })
    ...
    private checkPublicationStatusAndUpdate(): Promise<void> {
      ...
      return new Promise(async (resolve, reject) => {
        Logger.log(
          'publicationservice',
          'Requesting identity publication status to Assist for confirmation ID ' +
            this.manager.persistentInfo.did.assist.publicationID
        );
        ...
        let assistAPIEndpoint = await this.getAssistEndpoint();
        this.http
          .get(assistAPIEndpoint + '/didtx/confirmation_id/' + this.manager.persistentInfo.did.assist.publicationID, {
            headers: headers
          })
          .toPromise()
          .then(async (response: AssistTransactionStatusResponse) => {
            Logger.log('publicationservice', 'Assist successful response:', response);
            if (response && response.meta && response.meta.code == 200 && response.data.status) {
              Logger.log(
                'publicationservice',
                'All good, We got a clear status from the assist api:',
                response.data.status
              );

              if (
                response.data.status == AssistTransactionStatus.PENDING ||
                response.data.status == AssistTransactionStatus.PROCESSING
              ) {
                ...
                Logger.log(
                  'publicationservice',
                  'Publication request was quarantined! Deleting the identity and trying again.'
                );
                this.manager.persistentInfo.did.publicationStatus = DIDPublicationStatus.FAILED_TO_PUBLISH;
                await this.manager.savePersistentInfoAndEmitStatus(this.manager.persistentInfo);
                ...
                Logger.error(
                  'publicationservice',
                  'Unhandled transaction status received from assist:',
                  response.data.status
                );
                this.manager.persistentInfo.did.publicationStatus = DIDPublicationStatus.FAILED_TO_PUBLISH;
                await this.manager.savePersistentInfoAndEmitStatus(this.manager.persistentInfo);
  ...
  export class WalletPublisher extends DIDPublisher {
    ...
    public async publishDID(
      ...
      try {
        ...
        let response = await this.globalIntentService.sendIntent(
          'https://wallet.web3essentials.io/didtransaction',
          params,
          parentIntentId
        );
    ...
    private async checkPublicationStatusAndUpdate(): Promise<void> {
      ...
      return new Promise(async (resolve, reject) => {
        Logger.log(
          'publicationservice',
          'Checking transaction status on chain for txid ' + this.manager.persistentInfo.did.wallet.txId
        );
        ...
        void this.jsonRPC.httpPost(endpoint, request).then(async (response: EIDChainResolveResponse) => {
          ...
          if (response && response.transaction && response.transaction.length > 0) {
            ...
            if ('0x' + response.transaction[0].txid === this.manager.persistentInfo.did.wallet.txId) {
              ...
              Logger.log(
                'publicationservice',
                'Txid is still the old one, we keep waiting.',
    ...
    private getEIDEndpoint(): string {
      return GlobalElastosAPIService.instance.getRPCApiUrlWithOverride(ElastosApiUrlType.EID_RPC);
    }
...
class DIDPublishingManager {
  ...
  private async loadPersistentInfo(): Promise<PersistentInfo> {
    let infoAsString = await this.storage.getSetting(
    ...
    if (!infoAsString) return this.createNewPersistentInfo();
  ...
  public async resetStatus() {
    if (this.activePublisher) await this.activePublisher.resetStatus();
  ...
  private async getPublishingMedium(): Promise<string> {
    if (!DIDSessionsStore.signedInDIDString) return 'assist'; // No signed in user? We may be in a DID creation flow. Anyway, use assist in this case.

    return await this.prefs.getPublishIdentityMedium(
      DIDSessionsStore.signedInDIDString,
      NetworkTemplateStore.networkTemplate
    );
  }

  public async publishDIDFromRequest(
    didString: string,
    payloadObject: JSONObject,
    memo: string,
    showBlockingLoader = false,
    ...
    if (publishMedium === 'assist') this.activePublisher = this.assistPublisher;
    else this.activePublisher = this.walletPublisher;
  ...
  public async displayPublicationLoader(): Promise<void> {
    const modal = await this.modalCtrl.create({
      ...
      cssClass: !this.theme.darkMode
        ? 'identity-showqrcode-component identity-publishmode-component-base'
        : 'identity-showqrcode-component-dark identity-publishmode-component-base',
...
export class GlobalPublicationService extends GlobalService {
   ...
   */
  public publishCordovaDIDFromStore(
    storeId: string,
    storePass: string,
    didString: string,
    showBlockingLoader = false,
  ...
  public publishJSDIDFromStore(
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      if (localDIDDocument) {
        ...
        try {
          await localDIDDocument.publish(
            storePass,
            undefined,
            true,
            new (class implements DIDTransactionAdapter {
              async createIdTransaction(payload: string, memo: string) {
                // Callback called by the DID SDK when trying to publish a DID.
                Logger.log('publicationservice', 'Create ID transaction callback is being called', payload, memo);
                const payloadAsJson = JSON.parse(payload);
                await publicationService.publishDIDFromRequest(
                  didString,
                  payloadAsJson,
                  memo,
                  showBlockingLoader,
  ...
  private openDidStore(
    storeId: string,
    createIdTransactionCallback: DIDPlugin.OnCreateIdTransaction
  ): Promise<DIDPlugin.DIDStore> {
    return new Promise(resolve => {
      didManager.initDidStore(
        storeId,
        createIdTransactionCallback,
        didstore => {
          resolve(didstore);
        },
        err => {
