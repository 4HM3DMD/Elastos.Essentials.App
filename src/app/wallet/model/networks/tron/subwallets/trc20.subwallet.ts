...
export class TRC20SubWallet extends SubWallet<TronTRC20Transaction, any> {
  ...
  public static async newFromCoin(
    networkWallet: TronNetworkWallet<MasterWallet, WalletNetworkOptions>,
    coin: Coin
  ...
  public static async newFromSerializedSubWallet(
    networkWallet: AnyTronNetworkWallet,
  ...
  /**
   * Creates a new TronWeb instance with fresh RPC URL. We recreate it each time to ensure
   * we always use the latest selected RPC URL instead of a cached one.
   */
  private async createTronWeb(): Promise<any> {
    const TronWeb = await lazyTronWebImport();
    const rpcUrl = this.networkWallet.network.getSelectedRpcUrl();
    const tronWeb = new TronWeb({
      fullHost: rpcUrl
    });
    tronWeb.setAddress(this.coin.trc20ContractAddress);
    return tronWeb;
  }

  /**
   * Creates a new contract handler with fresh RPC URL. We recreate it each time to ensure
   * we always use the latest selected RPC URL instead of a cached one.
   */
  private async createContractHandler(): Promise<any> {
    const tronWeb = await this.createTronWeb();
    return await tronWeb.contract().at(this.coin.trc20ContractAddress);
  }
  ...
  public async updateBalance(): Promise<void> {
    ...
    try {
      ...
      const contractHandler = await this.createContractHandler(); // Create fresh contract handler to get latest RPC URL
      const rawBalance = await contractHandler.balanceOf(tokenAccountAddress).call();
      if (rawBalance) {
  ...
  public async getTransactionDetails(txid: string): Promise<TronTransactionInfo> {
    return await GlobalTronGridService.instance.getTransactionInfoById(
  ...
  public async getTransactionInfo(transaction: TronTRC20Transaction): Promise<TransactionInfo> {
    ...
    void this.networkWallet.getOrFetchExtendedTxInfo(transaction.transaction_id).then(async extInfo => {
      if (extInfo && extInfo.tvm && extInfo.tvm.txInfo) {
        if (extInfo.tvm.txInfo.fee)
          (transactionInfo.fee = new BigNumber(extInfo.tvm.txInfo.fee)
  ...
  private getTransactionResourcesConsumed(transaction: TronTransactionInfo) {
    ...
    if (transaction.receipt.net_usage) {
      resourcesString +=
        transaction.receipt.net_usage +
        ' ' +
        GlobalTranslationService.instance.translateInstant('wallet.tx-info-resource-bandwidth');
    }
    ...
    if (transaction.receipt.energy_usage_total) {
      ...
      resourcesString +=
        transaction.receipt.energy_usage_total +
        ' ' +
        GlobalTranslationService.instance.translateInstant('wallet.tx-info-resource-energy');
    }
  ...
  public async estimateTransferTransactionGas() {
    ...
    let result = await GlobalTronGridService.instance.triggerConstantContract(
      contractAddress,
      100000,
      tokenAccountAddress
    );
    ...
    if (this.contractEnergyFactor == -1) {
      let contractInfo = await GlobalTronGridService.instance.getContractInfo(
        this.networkWallet.network.getSelectedRpcUrl(),
        contractAddress
      );
  ...
  public async createPaymentTransaction(toAddress: string, amount: BigNumber, feeLimit: number = null): Promise<any> {
    ...
    Logger.log(
      'wallet',
      'createPaymentTransaction toAddress:',
      toAddress,
      ' amount:',
      amount.toString(),
      ' feeLimit:',
    ...
    const tronWeb = await this.createTronWeb(); // Create fresh tronWeb instance to get latest RPC URL
    let result = await tronWeb.transactionBuilder.triggerSmartContract(
      contractAddress,
  ...
  protected async sendRawTransaction(payload: string) {
    return await GlobalTronGridService.instance.sendrawtransaction(
      this.networkWallet.network.getSelectedRpcUrl(),
      payload
    );
  }
  ...
  protected async markGenericOutgoingTransactionEnd(txid: string, message: string = '') {
    if (txid) {
      ...
      if (extInfo && extInfo.tvm && extInfo.tvm.txInfo && extInfo.tvm.txInfo.receipt.result !== 'SUCCESS') {
        TransactionService.instance.setOnGoingPublishedTransactionState(
          OutgoingTransactionState.ERRORED,
