...
export class BTCSubWallet extends MainCoinSubWallet<BTCTransaction, any> {
  ...
  public async getAvailableUtxo(amount: number, feeInSatPerKB: number, useInscriptionUTXO = false): Promise<BTCUTXO[]> {
    ...
    if (!useInscriptionUTXO) {
      ...
      inscriptionUTXO.utxo.forEach(utxo => {
        utxoArray.splice(
          utxoArray.findIndex(u => u.txid === utxo.txid),
          1
        );
      });
    }
    ...
    if (this.unconfirmedSendingTxids.length > 0) {
      ...
      for (let index = 0; index < this.unconfirmedSendingTxids.length; index++) {
        ...
        if (!isTxInCache) {
          rawTx = await GlobalBTCRPCService.instance.getrawtransaction(
            this.explorerApiUrl,
            this.unconfirmedSendingTxids[index]
          );
        }
      ...
      if (utxosInSpending.length > 0) {
        utxosInSpending.forEach(tx => {
          utxoArray.splice(
            utxoArray.findIndex(u => u.txid === tx),
            1
          );
        });
      }
  ...
  private async estimateFeeRate(feeRate = BTCFeeSpeed.AVERAGE, forcedSatPerKB = null): Promise<number> {
    if (forcedSatPerKB) return forcedSatPerKB;

    const btcPerKB = await GlobalBTCRPCService.instance.estimatesmartfee(
      this.networkWallet.network.getSelectedRpcUrl(),
      feeRate
    );
    if (!btcPerKB) throw new Error('BTCSubWallet: Failed to estimate fee rate by API');
  ...
  public async estimateTransferTransactionGas(
    feeSpeed = BTCFeeSpeed.AVERAGE,
    forcedSatPerKB = null,
    amountBTC: BigNumber = null,
    useInscriptionUTXO = false
  ) {
    ...
    if (!amountBTC) return satPerKB;

    let result = await this.getUTXOsAndFee(amountBTC, satPerKB, useInscriptionUTXO);
    if (!result) throw new Error('BTCSubWallet: Utxo is not enough');
  ...
  public async createPaymentTransaction(
    ...
    Logger.log(
      'wallet',
      'BTCSubWallet: createBTCTransaction  toAddress:',
      toAddress,
      ' amount:',
      toAmount,
      ' feeSat:',
  ...
  protected async sendRawTransaction(payload: string) {
    return await GlobalBTCRPCService.instance.sendrawtransaction(
      this.networkWallet.network.getSelectedRpcUrl(),
      payload
    );
  }
  ...
  private async updateBTCSubWalletInfo() {
    // Get the latest info.
    let btcInfo = await GlobalBTCRPCService.instance.address(
      this.explorerApiUrl,
      this.btcAddress,
      TRANSACTION_LIMIT,
      1
