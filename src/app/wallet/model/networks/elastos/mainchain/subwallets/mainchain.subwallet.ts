...
const voteTypeMap = [
  VoteTypeString.Delegate,
  VoteTypeString.CRC,
  VoteTypeString.CRCProposal,
  VoteTypeString.CRCImpeachment
];
...
export class MainChainSubWallet extends MainCoinSubWallet<ElastosTransaction, ElastosMainChainWalletNetworkOptions> {
  ...
  public async destroy() {
    ...
    // delete cache
    if (this.stakedBalanceCache) await this.stakedBalanceCache.delete();

    if (this.mintNFTTxCache) await this.mintNFTTxCache.delete();
  }

  public supportsCrossChainTransfers(): boolean {
    // Not support cross chain transfers for gnosis muti-sig and ledger wallet.
    return (
      this.networkWallet.masterWallet.type == WalletType.STANDARD ||
  ...
  public isSingleAddress(): boolean {
    if (!this.networkWallet.getNetworkOptions()) return false;
  ...
  public async createDepositTransaction(
    toSubWalletId: StandardCoinName,
    toAddress: string,
    amount: number,
    memo = ''
  ): Promise<string> {
  ...
  public createConsolidateTransaction(utxoArray: Utxo[], memo = ''): Promise<string> {
    ...
    let max_Consolidate_Utxos = (
      this.networkWallet.safe as unknown as ElastosMainChainSafe
    ).getMaxUtxoConsolidateCount();
  ...
  public CRCouncilMemberClaimNodeDigest(payload: CRCouncilMemberClaimNodeInfo, version: number): string {
    ...
    return (this.networkWallet.safe as unknown as ElastosMainChainSafe).CRCouncilMemberClaimNodeDigest(
      payload,
      version
    );
  }
    ...
    for (let i = 0, len = transactions.length; i < len; i++) {
      ...
      if (transactions[i].Status !== TransactionStatus.CONFIRMED) {
        if (
          !bposTx ||
          [
            RawTransactionType.Stake,
            RawTransactionType.Unstake,
            RawTransactionType.Voting,
            RawTransactionType.MintNFT
          ].includes(transactions[i].txtype)
        ) {
  ...
  private async updatePendingTransaction() {
    ...
    if (invalidPendingTransactions.length > 0) {
      ...
      await this.networkWallet
        .getTransactionDiscoveryProvider()
        .removeTransactionsFromCache(this, invalidPendingTransactions);
    }
  ...
  public async getAvailableUtxo(amountSELA: number, address = null): Promise<AvalaibleUtxos> {
    ...
    if (this.id === StandardCoinName.ELA) {
      if (address) {
        // for createStakeTransaction. Only use the utxos of the first external address.
  ...
  private async mergeVoteContents(userVoteContents: VoteContentInfo[]) {
    ...
    let newVoteContents = await this.invalidVoteCandidatesHelper.removeInvalidCandidates(
      userVoteContents,
      votedContents
    );
  ...
  private async loadStakedBalanceFromCache() {
    if (!this.stakedBalanceKeyInCache) {
      this.stakedBalanceKeyInCache =
        this.masterWallet.id + '-' + this.getUniqueIdentifierOnNetwork() + '-stakedbalance';
    }
  ...
  public async callGetBalanceByAddress(
    subWalletId: StandardCoinName,
    addressArray: string[],
    spendable = false
  ): Promise<BalanceList> {
    let apiurltype = GlobalElastosAPIService.instance.getApiUrlTypeForRpc(subWalletId);
      ...
      if (txDetail.payload) {
        ...
        if (txDetail.payload.crosschainaddresses) {
          ...
          if (txDetail.payload.genesisblockaddress === Config.ETHSC_DEPOSIT_ADDRESS) {
            ...
            Logger.error(
              'wallet',
              'Can not find the chain for genesis block address:',
              txDetail.payload.genesisblockaddress
            );
            return '';
          }
        }
  ...
  async getAllUtxoByType(type: UtxoType, addressArray: string[] = null) {
    ...
    if (addressArray && addressArray.length > 0) {
      utxoArray = await GlobalElastosAPIService.instance.getAllUtxoByAddress(
        this.id as StandardCoinName,
        addressArray,
        type
      );
      ...
      if (!elastosNetworkOptions.singleAddress) {
        ...
        if (utxos && utxos.length > 0) {
          if (utxoArray) utxoArray = [...utxoArray, ...utxos];
          else utxoArray = utxos;
        }
      }
    }
    return utxoArray;
  ...
  async getUtxosByAmount(address: string, amountELA: string, utxotype: UtxoType) {
    let utxoArray: Utxo[] = null;
    try {
      utxoArray = await GlobalElastosAPIService.instance.getUtxosByAmount(
        this.id as StandardCoinName,
        address,
        amountELA,
        utxotype
  ...
  getAddressListByAmount(amountSELA: string, filterUtxo: Utxo[] = null): string[] {
    ...
    for (let i = 0; filterUtxo && i < filterUtxo.length; i++) {
      ...
      if (addresses) {
        addresses.value = addresses.value.minus(
          new BigNumber(filterUtxo[i].amount).multipliedBy(this.tokenAmountMulipleTimes)
        );
      }
    }
  ...
  async getAllUtxoByAddress(internalAddress: boolean, type: UtxoType): Promise<Utxo[]> {
    ...
    do {
      ...
      try {
        let utxos = await GlobalElastosAPIService.instance.getAllUtxoByAddress(
          this.id as StandardCoinName,
          addressArray,
          type
        );
  ...
  public proposalSecretaryGeneralElectionCRCouncilMemberDigest(payload: SecretaryElectionInfo): string {
    return (
      this.networkWallet.safe as unknown as ElastosMainChainSafe
    ).proposalSecretaryGeneralElectionCRCouncilMemberDigest(payload);
  }
  ...
  public proposalChangeOwnerCRCouncilMemberDigest(payload: ChangeProposalOwnerInfo): string {
    return (this.networkWallet.safe as unknown as ElastosMainChainSafe).proposalChangeOwnerCRCouncilMemberDigest(
      payload
    );
  }
  ...
  public async createDPoSV2VoteTransaction(payload: VotingInfo, memo = ''): Promise<EncodedTx> {
    ...
    return (this.networkWallet.safe as unknown as ElastosMainChainSafe).createDPoSV2VoteTransaction(
      au.utxo,
      payload,
      '10000',
      memo
  ...
  public async createDPoSV2ClaimRewardTransaction(payload: DPoSV2ClaimRewardInfo, memo = ''): Promise<EncodedTx> {
    ...
    return (this.networkWallet.safe as unknown as ElastosMainChainSafe).createDPoSV2ClaimRewardTransaction(
      au.utxo,
      payload,
      '10000',
      memo
  ...
  public async createUnstakeTransaction(payload: UnstakeInfo, memo = ''): Promise<EncodedTx> {
    ...
    return (this.networkWallet.safe as unknown as ElastosMainChainSafe).createUnstakeTransaction(
      au.utxo,
      payload,
      '10000',
      memo
  ...
  public async createMintNFTTransaction(payload: CreateNFTInfo, memo = ''): Promise<EncodedTx> {
    ...
    return (this.networkWallet.safe as unknown as ElastosMainChainSafe).createMintNFTTransaction(
      au.utxo,
      payload,
      '10000',
      memo
