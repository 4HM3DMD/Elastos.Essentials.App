...
export abstract class MasterWallet {
  ...
  public getNetworkOptions(networkKey: string): WalletNetworkOptions {
    let defaultNetworkOptions = WalletNetworkService.instance
      .getNetworkByKey(networkKey)
      .getDefaultWalletNetworkOptions();
  ...
  /**
   * Returns all networks supported by this master wallet
   * @param networkTemplate Optional network template to filter by
   * @returns Array of supported networks
   */
  public getSupportedNetworks(networkTemplate?: string): AnyNetwork[] {
    return WalletNetworkService.instance.getAvailableNetworks(this, networkTemplate);
  }
}
...
export class StandardMasterWallet extends MasterWallet {
  ...
  public async getSeed(decryptedWithPayPassword?: string): Promise<string> {
    if (!this.getSafe().seed) return null;

    if (!decryptedWithPayPassword) return await this.getSafe().seed;
    else return AESDecrypt(this.getSafe().seed, decryptedWithPayPassword);
  }
  ...
  public async getMnemonic(decryptedWithPayPassword?: string): Promise<string> {
    ...
    if (!decryptedWithPayPassword) return await this.getSafe().mnemonic;
    else return AESDecrypt(this.getSafe().mnemonic, decryptedWithPayPassword);
  }
  ...
  public async getPrivateKey(decryptedWithPayPassword?: string): Promise<string> {
    ...
    if (!decryptedWithPayPassword) return await this.getSafe().privateKey;
    else return AESDecrypt(this.getSafe().privateKey, decryptedWithPayPassword);
  }
  ...
  public async setBitcoinAddressType(addressType: BitcoinAddressType) {
    let btcNetworkOptions = this.networkOptions.find(
      no => no.network === BTCNetworkBase.networkKey
    ) as BTCWalletNetworkOptions;
