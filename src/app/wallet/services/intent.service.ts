...
export class IntentService {
  ...
  async onReceiveIntent(intent: EssentialsIntentPlugin.ReceivedIntent) {
    if (
      intent.action.indexOf('https://wallet.elastos.net/') != 0 &&
      intent.action.indexOf('https://wallet.web3essentials.io/') != 0
    )
      return; // Not for us.
    ...
    if (!this.activeWallet) {
      ...
      if (toCreateWallet) {
        ...
        await this.globalIntentService.sendIntentResponse(
          { message: 'No active master wallet!', status: 'error' },
          intent.intentId
        );
      }
      return false;
  ...
  handleAddCoinIntent(intent: EssentialsIntentPlugin.ReceivedIntent) {
    ...
    this.native.setRootRouter('/wallet/coin-add-erc20', {
      ...params,
      forIntent: true
    });
  }
  ...
  private handleVoteAgainstProposalIntent(intent: EssentialsIntentPlugin.ReceivedIntent) {
    ...
    this.coinTransferService.transfer.votes = [intent.params.proposalHash];
  }
  ...
  private async handleCreateProposalDigestIntent(intent: EssentialsIntentPlugin.ReceivedIntent) {
    ...
    if (intent && intent.params && intent.params.proposal) {
      ...
      if (mainChainSubwallet) {
        ...
        await this.globalIntentService.sendIntentResponse(
          { message: 'No ELA Main chain wallet', status: 'error' },
          intent.intentId
        );
      }
    } else {
      ...
      await this.globalIntentService.sendIntentResponse(
        {
          message: 'Missing proposal input parameter in the intent',
          status: 'error'
        },
        intent.intentId
      );
  ...
  private async handlePayIntent(intent: EssentialsIntentPlugin.ReceivedIntent): Promise<boolean> {
    ...
    if (intentSubWalletId) {
      ...
      await this.globalIntentService.sendIntentResponse(
        {
          message: 'Not support Token:' + intent.params.currency,
          status: 'error'
        },
        intent.intentId
      );
