...
class _Logger implements GlobalService {
  ...
  async onUserSignIn(signedInIdentity: IdentityEntry): Promise<void> {
    ...
    let collectLogs = await GlobalPreferencesService.instance.getCollectLogs(
    ...
    this.prefSub = GlobalPreferencesService.instance.preferenceListener.subscribe(pref => {
      if (pref.key === 'developer.collectLogs') {
        this.setCollectDevLogs(pref.value as boolean);
      }
    });
  }
  ...
  public log(module: string, ...args: any) {
    ...
    this.originalDebugLog.apply(this.originalConsole, [
      '%c' + moment(new Date().getTime()).format('HH:mm:ss.SSS') + ' ' + module.toUpperCase() + '*',
      'background: #008730; color: #FFF; font-weight:bold; padding:5px;',
      ...args
    ]);
  }
  ...
  public warn(module: string, ...args: any) {
    ...
    this.originalDebugWarn.apply(this.originalConsole, [
      '%c' + moment(new Date().getTime()).format('HH:mm:ss.SSS') + ' ' + module.toUpperCase() + '* WARNING',
      'background: #d59100; color: #FFF; font-weight:bold; padding:5px;',
      ...args
    ]);
  }
  ...
  public error(module: string, ...args: any) {
    ...
    this.originalDebugErr.apply(this.originalConsole, [
      '%c' + moment(new Date().getTime()).format('HH:mm:ss.SSS') + ' ' + module.toUpperCase() + '* ERROR',
      'background: #b30202; color: #FFF; font-weight:bold; padding:5px;',
      ...args
    ]);
  }
  ...
  public test(module: string, ...args: any) {
    ...
    this.originalDebugLog.apply(this.originalConsole, [
      '%c' + moment(new Date().getTime()).format('HH:mm:ss.SSS') + ' ' + module.toUpperCase() + '* TEST',
      'background: #7B68EE; color: #FFF; font-weight:bold; padding:5px;',
      ...args
    ]);
  }
