...
import {
  ...
  Utxo,
  UtxoType
} from 'src/app/wallet/model/tx-providers/transaction.types';
...
export class CRCouncilService {
  ...
  async fetchCRMembers() {
    ...
    try {
      ...
      for (let item of resultFromCR.data.council) {
        ...
        if (crmember) crmember.didName = item.didName;
      }
    } catch (err) {
  ...
  async getSelectedCandidates(): Promise<any> {
    let data = await this.storage.getSetting(
  ...
  async setSelectedCandidates(selectedCandidates: SelectedCandidate[]): Promise<any> {
    return await this.storage.setSetting(
      DIDSessionsStore.signedInDIDString,
      NetworkTemplateStore.networkTemplate,
      'crcouncil',
      this.voteService.masterWalletId + 'votes',
  ...
  async fetchNodeAvatars() {
    this.logoUrl = GlobalElastosAPIService.instance.getRPCApiUrlWithOverride(ElastosApiUrlType.IMAGES) + '/';
    try {
  ...
  async addCandidateOperationIcon(
    darkMode: boolean,
    titleBar: TitleBarComponent,
    titleBarIconClickedListener: (icon: TitleBarIcon | TitleBarMenuItem) => void
  ) {
    if (this.candidateInfo.state == 'Active' || this.candidateInfo.state == 'Pending') {
        ...
        [
          {
            key: 'edit-candidate',
            iconPath: !darkMode
              ? '/assets/crcouncilvoting/icon/edit-candidate.svg'
              : '/assets/crcouncilvoting/icon/darkmode/edit-candidate.svg',
            title: 'crcouncilvoting.edit-candidate'
          },
          {
            key: 'cancel-registration',
            iconPath: !darkMode
              ? '/assets/crcouncilvoting/icon/unregister-candidate.svg'
              : '/assets/crcouncilvoting/icon/darkmode/unregister-candidate.svg',
      ...
      titleBar.addOnItemClickedListener(
        (titleBarIconClickedListener = item => {
          switch (item.key) {
            case 'edit-candidate':
      ...
      if (available > 0) {
        titleBar.setIcon(TitleBarIconSlot.OUTER_RIGHT, {
          key: null,
          iconPath: this.theme.darkMode
            ? '/assets/crcouncilvoting/icon/darkmode/withdraw.svg'
            : '/assets/crcouncilvoting/icon/withdraw.svg'
        });
        titleBar.addOnItemClickedListener(
          (titleBarIconClickedListener = icon => {
  ...
  async unregisterCandidate() {
    ...
    if (
      !(await this.globalPopupService.ionicConfirm(
        'common.warning',
        'crcouncilvoting.candidate-unregister-warning',
        'common.confirm',
        'common.cancel'
      ))
    ) {
    ...
    try {
      ...
      if (payload) {
        ...
        if (signature) {
          ...
          let ret = await this.voteService.signAndSendRawTransaction(
            rawTx,
            App.CRCOUNCIL_VOTING,
            '/crcouncilvoting/candidates'
          );
          if (ret) {
  ...
  async withdrawCandidate(available: number, customRoute?: string) {
    ...
    if (!(await this.voteService.isSamePublicKey())) {
      void this.globalPopupService.ionicAlert(
        'common.warning',
        'crcouncilvoting.use-registered-wallet',
        'common.understood'
      );
      return;
    }
    ...
    let msg =
      this.translate.instant('crcouncilvoting.candidate-withdraw-warning-pre') +
      available +
    ...
    try {
      ...
      let utxoArray = (await GlobalElastosAPIService.instance.getAllUtxoByAddress(
  ...
  async getCRDepositcoin(): Promise<any> {
    ...
    try {
      const result = await this.jsonRPCService.httpPost(
        this.voteService.getElaRpcApi(),
        param,
        'default',
        10000,
        false,
