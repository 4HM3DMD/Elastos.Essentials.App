...
import {
  ElastosApiUrlType,
  GlobalElastosAPIService,
  ImageInfos,
  NodeType
} from 'src/app/services/global.elastosapi.service';
...
export class DPoS2Service {
      ...
      if (result && !Util.isEmptyObject(result.producers)) {
        ...
        for (const node of this._nodes) {
          ...
          if (node.state === 'Active' || node.state === 'Inactive') {
            ...
            if (until < 720 * 30 && node == this.dposInfo) {
              //less than 30 days
              if (until <= 15) {
  ...
  async fetchMyVoteds(): Promise<any[]> {
    ...
    let rpcApiUrl = this.globalElastosAPIService.getRPCApiUrlWithOverride(ElastosApiUrlType.ELA_RPC);
