...
export class WidgetsFeedsNewsService implements GlobalService {
  ...
  public async fetchedSubscribedChannels(onlyExpired = true) {
    ...
    if (this.fetchingChannels.value) return;
    ...
    for (let feedsChannel of feedsChannels) {
      let lastFetched = await this.globalStorageService.getSetting(
        signedInUserDid,
        NetworkTemplateStore.networkTemplate,
        PERSISTENCE_CONTEXT,
        feedsChannel.getChannelId() + '_lastfetched',
        0
      );
      let rightTimeToRefresh = moment
        .unix(lastFetched)
        .add(MIN_CHANNEL_REFRESH_DELAY_SECS, 'seconds')
        .isSameOrBefore(lastFetched);
      if (!rightTimeToRefresh && onlyExpired) {
        ...
        if (channel) {
          // Make sure we really have it in cache
    ...
    for (let channel of channels) {
      ...
      if (existingChannel) channel.enabled = existingChannel.enabled;
    }
  ...
  private async loadChannels(signedInUserDid: string): Promise<void> {
    const key = 'widget-feeds-channels';
    const sources = await this.globalStorageService.getSetting(
      signedInUserDid,
      NetworkTemplateStore.networkTemplate,
      PERSISTENCE_CONTEXT,
      key,
