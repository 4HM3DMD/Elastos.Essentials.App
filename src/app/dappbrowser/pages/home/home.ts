...
export class HomePage {
  //implements DappBrowserClient // '_blank' mode {
  ...
  private async buildFilteredDApps() {
    ...
    this.dApps = this.allDApps.filter(app => {
      ...
      if (!canBrowseInApp && !app.walletConnectSupported) return false;
  ...
  public getRecentAppOrFavoriteDescription(recentAppOrFavorite: BrowsedAppInfo | BrowserFavorite): string {
    if (!recentAppOrFavorite.description) return '';

    const limit = 50;
    if (recentAppOrFavorite.description.length < limit) return recentAppOrFavorite.description;
    else return recentAppOrFavorite.description.substring(0, limit) + '...';
  }

  public pickNetwork() {
    void this.walletNetworkUIService.chooseActiveNetwork(this.createNetworkFilter());
  }

  /**
   * Creates a network filter that only shows networks supported by the currently connected EVM wallet.
   * Since this is the home page (not browsing a specific dapp), we don't filter networks.
   */
  private createNetworkFilter(): NetworkChooserFilter | undefined {
    // On the home page, we don't have a specific dapp context, so show all networks
    return undefined;
  }
  ...
  public async toggleRecentAppFavorite(event, recentApp: BrowsedAppInfo) {
    ...
    if (favorite) await this.favoritesService.removeFromFavorites(favorite);
    else await this.favoritesService.addToFavorites(recentApp);
  }

  private async checkNoInAppNoticeStatus(): Promise<void> {
    this.noInAppNoticeDismissed = await this.globalStorageService.getSetting(
      DIDSessionsStore.signedInDIDString,
      NetworkTemplateStore.networkTemplate,
      'dappbrowser',
      'noinappnoticedismissed',
      false
    );
  }
