...
export class URLInputAssistantComponent {
  ...
  private prepareRecentApps() {
    if (!this._filter) return (this.recentApps = this.rawRecentApps); // No filter; return everything
  ...
  public getShortRecentAppDescription(recentApp: BrowsedAppInfo): string {
    if (!recentApp.description) return '';

    const limit = 50;
    if (recentApp.description.length < limit) return recentApp.description;
    else return recentApp.description.substring(0, limit) + '...';
  }
